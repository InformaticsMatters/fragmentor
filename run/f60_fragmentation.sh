...
time nextflow run -c $REPPATH/nextflow/nextflow.config $REPPATH/nextflow/fragmentation.nf -with-docker \
    --input $REPPATH/$FRAGBASEDIR/$FRAGSMIFILE --out_dir $REPPATH/$FRAGBASEDIR --chunk_size $FRAGCHUNKSIZE --max_hac $FRAGHAC --max_frag $FRAGMAXFRAGS $@
